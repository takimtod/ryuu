...
async function imageToWebp(media) {
   const tmpFileIn = path.join('./', `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.${media?.ext || "png"}`)
 ...
 async function videoToWebp(media) {
   const tmpFileIn = path.join('./', `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.${media?.ext || "mp4"}`)
   const tmpFileOut = path.join('./', `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`)
